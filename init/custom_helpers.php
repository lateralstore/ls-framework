<?php
	 ...
	 */
	function trace_log($Str, $Listener = 'INFO')
	{
		$trace_log = isset(Phpr::$traceLog) ? Phpr::$traceLog : Phpr::$trace_log;
		if ($trace_log)
			$trace_log->write($Str, $Listener);
	}
	...
	/**
	 * Returns file URL relative to the site root.
	 * The function makes links independent of the actual site installation directory name, whenever it
	 ...
	 * your site installation from a subdirectory to the domain root directory. <span class="note">Using this function
	 * is required when creating themes.</span>
	 * For example, if you installed your site into the shop subdirectory of <em>http://my_host.com</em> domain, the following call
	 * will return <em>/shop/cart</em> URL:
	 * <pre>root_url('/cart')</pre>
	 * If you move the site installation to the domain root directory the same function call will return <em>/cart</em> URL, so that you
	 * do not need to update your links manually.
	 * @documentable
	 * @package cms.helpers
	 * @see site_url()
	 * @param string $value Specifies the URL to process.
	 * @param string $add_host`_name_and_protocol Indicates whether the URL should contain the host name and protocol.
	 * @param string $protocol Optional HTTP protocol name to override the actual protocol name.
	 * @return string Returns an URL of a specified resource relative to the site domain root.
	 */
	 ...
	 * @deprecated
	 */
	function traceLog($Str, $Listener = 'INFO'){trace_log($Str, $Listener = 'INFO');}
