...
class Phpr_Cron
{
	...
	public static function update_interval($code)
	{
		...
		Db_DbHelper::query('insert into phpr_cron_table (record_code, updated_at) values (:record_code, now()) on duplicate key update updated_at =:now', $bind);
	}

	public static function get_interval($code)
	{
		$interval = Db_DbHelper::scalar('select updated_at from phpr_cron_table where record_code =:record_code', array('record_code'=>$code));
		if (!$interval)
		{
	...
	public static function queue_job($handler_name, $param_data=array())
	{
		...
		Db_DbHelper::query('insert into phpr_cron_jobs (handler_name, param_data, created_at) values (:handler_name, :param_data, now())', $bind);
	}
